...
int aoc(FILE *input) {
  ...
  for(int i = 0; i < 7; i++) {
    if(-pairs[i].unlock_off <= pairs[i].lock_off) {
      serial[pairs[i].lock_pos] = '1';
      serial[pairs[i].unlock_pos] = '1' + pairs[i].lock_off + pairs[i].unlock_off;
    } else {
      serial[pairs[i].lock_pos] = '1' - pairs[i].lock_off - pairs[i].unlock_off;
      serial[pairs[i].unlock_pos] = '1';
    }
  }
